...
public class VBOHandler {
    ...
    public void render() {
        for (int i = 0; i < vertexHandler.size(); i++) {
            ...
            GL11.glDrawArrays(GL11.GL_TRIANGLE_STRIP, 0, length);
    ...
    public void popQueue() {
        while (!chunksToProcess.isEmpty()) {
            ...
            System.out.println("Genning buffer with length of " + worldCoords.size());
            ...
            for (int y = 0; y < Chunk.SECTION_HEIGHT; y++) {
                for (int z = 0; z < Chunk.CHUNK_WIDTH; z++) {
                    for (int x = 0; x < Chunk.CHUNK_WIDTH; x++) {
                        ...
                        // left side
                        if (x == 0 || currSection[index - 1].getID() == 0) {
                            drawSide(worldCoords, colorCoords, x + absoluteChunkX, y + sectionYOffset, z + absoluteChunkZ,
                                    leftSideTriangles);
                        }
                        // right side
                        if (x == chunkLimitSidewards || currSection[index + 1].getID() == 0) {
                            drawSide(worldCoords, colorCoords, x + absoluteChunkX, y + sectionYOffset, z + absoluteChunkZ,
                                    rightSideTriangles);
                        }
                        // back side
                        if (z == 0 || currSection[index - Chunk.CHUNK_WIDTH].getID() == 0) {
                            drawSide(worldCoords, colorCoords, x + absoluteChunkX, y + sectionYOffset, z + absoluteChunkZ,
                                    backSideTriangles);
                        }
                        // front side
                        if (z == chunkLimitSidewards || currSection[index + Chunk.CHUNK_WIDTH].getID() == 0) {
                            drawSide(worldCoords, colorCoords, x + absoluteChunkX, y + sectionYOffset, z + absoluteChunkZ,
                                    frontSideTriangles);
                        }
                        // top side
                        ...
                        if (y == chunkLimitUpwards
                                || (useNextTop && (upperSection == null || upperSection[topIndex].getID() == 0))
                                || (!useNextTop && currSection[topIndex].getID() == 0)) {
                            drawSide(worldCoords, colorCoords, x + absoluteChunkX, y + sectionYOffset, z + absoluteChunkZ,
                                    topSideTriangles);
                        }
                        // bottom side
                        ...
                        if (y == 0 || (useNextBottom && (lowerSection == null || lowerSection[botIndex].getID() == 0))
                                || (!useNextBottom && currSection[botIndex].getID() == 0)) {
                            drawSide(worldCoords, colorCoords, x + absoluteChunkX, y + sectionYOffset, z + absoluteChunkZ,
                                    bottomSideTriangles);
                        }
                    }
    ...
    private void drawSide(List<Float> worldCoordList, List<Float> colorCoords, int x, int y, int z,
        ...
        for (int i = 0; i < rawTriangleCoords.length; i += 3) {
            ...
            multipliedTriangleCoords[i + 1] = rawTriangleCoords[i] + y;
            multipliedTriangleCoords[i + 2] = rawTriangleCoords[i] + z;
        }

        // we are drawing 2 triangles, so we insert vertices in the order: 1, 2, 3 , 2, 3, 4
        for (int i = 0; i < 3 * 3; i++) {
